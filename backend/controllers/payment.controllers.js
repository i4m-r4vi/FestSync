...
export const createPaymentIntent = async (req, res) => {
  try {
    ...
    const alreadyRegistered = userReg.registeredEvents.some((element) =>
      element.eventId.toString() === regEvent
    )
    if (alreadyRegistered) {
      return res.status(404).json({ error: "You Already Registered for the event" });
    }
    await userReg.registeredEvents.push({ //UserAuth
      eventId: regEvent,
      eventName: getEvent.title,
      subEvent,
      eventDate: getEvent.EventDate
    })
    await getEvent.registeredUsers.push({ //Events
      userId: user._id,
      name: user.fullname,
      eventName: getEvent.title,
      mail:user.email,
